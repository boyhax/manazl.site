...
// }
function getplace(georesult, place) {
  let newplace = {
...
async function fetchData() {
  let states = require("./omancities.json");

  states.forEach(async (state) => {
    let address = await geocode(state.name.en);
    let image = await getPlacePhoto(address.place_id);
    let blob = await downloadImageAsBlob(image);
    const { error } = await supabase
      .from("places")
      .update({ ...getplace(address, state), image: blob }).eq('short_name',state.short_name);

    !error && console.log("upaated :>> ", getplace(address, state));
    error && console.log("error :>> ", error);
    state.cities.forEach(async (city) => {
      let address = await geocode(city.name.en);
      let image = await getPlacePhoto(address.place_id);
      let blob = await downloadImageAsBlob(image);
      const { error } = await supabase
        .from("places")
        .update({ ...getplace(address, city), image: blob }).eq('short_name',city.short_name);
      !error && console.log("upaated :>> ", getplace(address, city));
    });
  });
