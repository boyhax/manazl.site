...
setDefaults({
  key: process.env.NEXT_PUBLIC_GOOGLE_MAPS, // Your API key here.
...
geocode(RequestType.LATLNG, "48.8583701,2.2922926", {
  ...
  outputFormat: OutputFormat.JSON, // Include address descriptor in response.
})
  .then(({ results }) => {
    const address = results[0].formatted_address;
...
//     import.meta.env.VITE_APP_google_key as string,
//     { language: "en", region: "om" }
//   );
