...
function useGallery({ path, onChange }: { path: string; onChange?: (v: string[]) => void }) {
  ...
  async function addPhotos(files: GalleryPhotos) {
    ...
    try {
      ...
      const newPaths = uploadResults.map((res) => res.path).filter(Boolean);
